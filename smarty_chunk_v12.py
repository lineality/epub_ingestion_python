...
def count_files(directory):
    ...
    print(f"Current File Population in the Land of {directory} observed to be at: {counter}")
...
def extract_text_from_txt(
    ...
    with open(text_file_path, 'r') as file:
        ...
        if text:
            ...
            # Save individual JSON file
            individual_json_path = os.path.join(output_json_dir, f"{os.path.splitext(text_file_path)[0]}.json")
            with open(individual_json_path, 'w') as f:
                json.dump({'text': text.strip()}, f, indent=4)
            ...
            print_and_log(f"{text_file_path} -> Faile, no text extracted", this_txt_output_dir_path)
...
def chunk_text(sentences, chunk_size, overlap_size=150):
    """
    with overlap
    """
    ...
    for sentence in sentences:
        ...
        # Case 3:  Chunk + sentence exceed limit, time to split
            ...
            # start the next chunk (with the last sentence)
            current_chunk = last_sentence + " " + sentence + " "

        last_sentence = sentence
        if len(sentence) > overlap_size:
            # just the last part
            last_sentence = sentence[-75:]
...
remove_small_files('txt_pool', 80)  
...
print(f"Smart-Chunk automatee document processing: Inner Elapsed time: {elapsed_time} seconds")
elapsed_time = end_time_outer - start_time_outer
print(f"Smart-Chunk automatee document processing: Outer Elapsed time: {elapsed_time} seconds")
